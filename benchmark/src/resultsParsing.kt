...
enum class Measure { TotalTime, Batches, UpdatableSwitches, UsesTagAndMatch }
...
fun handleResultsOurs(outputDir: Path): List<TestData> =
    parseResults(outputDir, listOf(
        ...
        { s -> Measure.Batches to """Minimum (\d+) batches required to satisfy the query!""".toRegex().firstGroup(s)?.toInt()},
